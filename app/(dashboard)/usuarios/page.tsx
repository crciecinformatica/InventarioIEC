  ...
  if (!canManage) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center">
        <h2 className="mb-2 text-lg font-semibold text-white">Acesso Restrito</h2>
          ...
          {!requestsReady ? (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="h-[92px] rounded-lg border border-slate-800 bg-slate-900/60">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {solicitacoes.map((solicitacao) => (
                <button
                  ...
                  className="rounded-lg border border-blue-950 bg-blue-950/20 p-3 text-left transition hover:border-blue-800 hover:bg-blue-950/30"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <h3 className="truncate text-sm font-semibold text-white">{solicitacao.nome}</h3>
                      <p className="truncate text-xs text-slate-500">{solicitacao.email}</p>
                    </div>
                    <span className={`shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase ${solicitacao.status === 'revisao' ? 'bg-amber-950 text-amber-300' : 'bg-blue-950 text-blue-300'}`}>
          ...
          <AnimatePresence initial={false}>
            {usuarios.map(usuario => {
              ...
              <motion.article
                ...
                className="rounded-lg border border-slate-800 bg-slate-900 p-4 shadow-sm shadow-slate-950/20"
              >
                <button type="button" onClick={() => setSelected(usuario)} className="w-full text-left">
                  <div className="flex items-start gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-slate-800 text-sm font-semibold text-slate-300">
                      {initials(usuario.nome)}
                    </div>
                    <div className="min-w-0">
                      <h3 className="truncate font-semibold text-white">{usuario.nome}</h3>
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs">
                  ...
                  <span className={`rounded-md px-2 py-1 ${usuario.ativo ? 'bg-emerald-950 text-emerald-300' : 'bg-red-950 text-red-300'}`}>
                    {usuario.ativo ? 'Ativo' : 'Inativo'}
                  </span>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  <div className="rounded-lg border border-slate-700 bg-slate-800/60 p-3">
                    <p className="text-lg font-semibold text-slate-100">{usuario.total_acoes ?? 0}</p>
                    <p className="text-xs text-slate-400">ações auditadas</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/60 p-3">
                    <p className="text-lg font-semibold text-slate-100">{usuario.forum_acoes ?? 0}</p>
                ...
                <div className="mt-4 flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-lg border border-slate-700 px-3 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-800"
                  >
                    Editar
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-lg bg-slate-800 px-3 py-2 text-sm font-medium text-slate-100 transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:bg-slate-900 disabled:text-slate-500 disabled:opacity-70"
                  >
      ...
      <AnimatePresence initial={false}>
        ...
        {selectedRequest && (
          <AnimatedDialogFrame
            ...
            className="max-w-lg rounded-xl border border-slate-800 p-0"
          >
            <div className="border-b border-slate-800 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-semibold text-white">{selectedRequest.nome}</h2>
            ...
            <div className="space-y-4 p-5">
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className="rounded-lg border border-red-900 px-4 py-2 text-sm font-medium text-red-400 transition hover:bg-red-950 disabled:opacity-60"
                >
                  Rejeitar
                </button>
                <button
